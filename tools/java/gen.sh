...
if [ "$#" -ne 2 ]; then
  echo "Usage: bazel run //tools/java:gen -- <year> <day>" >&2
  exit 1
fi
...
sed \
  ...
  tools/java/templates/Solver.java.template \
...
sed \
  -e "s/{{YEAR}}/$YEAR/g" \
  -e "s/{{DAY_PADDED}}/$DAY_PADDED/g" \
  tools/java/templates/SolverTest.java.template \
...
sed \
  -e "s/{{YEAR}}/$YEAR/g" \
  -e "s/{{DAY}}/$DAY/g" \
  tools/java/templates/BUILD.bazel.template \
