...
public class Solver extends AbstractSolver {
  ...
  public String solvePart2() {
    ...
    for (int row = 0; row < this.grid.length; row++) {
      for (int col = 0; col < this.grid[0].length; col++) {
        if (this.grid[row][col] == EMPTY_SPACE && !this.startingPoint.equals(new Point(row, col))) {
          Direction currentDirection = Direction.NORTH;
          Point currentPosition = this.startingPoint.copy();
          HashSet<PointAndDirection> visited = new HashSet<>();
          this.grid[row][col] = OBSTACLE;
          while (!isOutsideGrid(currentPosition)) {
            PointAndDirection pad = new PointAndDirection(currentPosition, currentDirection);
  ...
  private boolean isOutsideGrid(Point p) {
    return p.row() < 0
        || p.row() >= this.grid.length
        || p.col() < 0
        || p.col() >= this.grid[0].length;
  }
