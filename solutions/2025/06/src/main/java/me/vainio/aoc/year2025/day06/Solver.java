  ...
  public Solver(final String input) {
    ...
    this.numbers =
        lines.subList(0, lines.size() - 1).stream()
            .map(line -> Arrays.stream(line.trim().split("\\s+")).map(Long::parseLong).toList())
            .toList();
    this.operations = Arrays.stream(lines.getLast().split("\\s+")).map(String::trim).toList();

    // Parse for part 2
    final List<String> subList = lines.subList(0, lines.size() - 1);
    final String transposed = Grid.ofChars(String.join("\n", subList)).transpose().toString();
    this.groups =
