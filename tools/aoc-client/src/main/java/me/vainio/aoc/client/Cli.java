...
final class Cli implements Runnable {
  ...
  @Option(
      names = {"-f", "--fetch"},
      description = "Fetch input data from AoC (default behavior).")
  private boolean fetchInput;

  @Option(
      names = {"-1", "--part1"},
      description = "Submit part 1 answer to Advent of Code.")
  private boolean postPart1;

  @Option(
      names = {"-2", "--part2"},
      description = "Submit part 2 answer to Advent of Code.")
  ...
  public void run() {
    ...
    if (year < minYear || year > maxYear) {
      throw new ParameterException(
          spec.commandLine(),
          String.format(
              "Invalid value '%d' for parameter 'year': " + "year must be between %d and %d.",
    ...
    if (day < minDay || day > maxDay) {
      throw new ParameterException(
          spec.commandLine(),
          String.format(
              "Invalid value '%d' for parameter 'day': " + "day must be between %d and %d.",
              day, minDay, maxDay));
    }
    ...
    if (year == maxYear) {
      ...
      if (today.getMonth() != Month.DECEMBER) {
        throw new ParameterException(
            spec.commandLine(), String.format("Advent of Code %d hasn't started yet.", year));
      }
      if (today.getDayOfMonth() < day) {
        throw new ParameterException(
            spec.commandLine(),
            String.format(
                "Day %d is not available yet (today is December %d).", day, today.getDayOfMonth()));
      }
    }
